                                     ...
                                     data = list(), hpc = c("none", "foreach"), ...)
{
  ...
  RVAL <- breakpoints.matrix(X, y, h = h, breaks = breaks, hpc = hpc, ...)
  fvals = fitted(RVAL, breaks=breaks, bp=RVAL$breakpoints)
...
{
  #obj input does not contain y value anymore
  X <- obj
  n <- nrow(X)
  k <- ncol(X)
  breakstat <- NULL
  ...
  return(RVAL)
} # end breakpoint.matrix function
...
breakpoints.breakpointsfull <- function(obj, breaks = c("BIC", "LWZ", "RSS", "all"), ...)
{
  ...
  class(RVAL) <- "breakpoints"
  return(RVAL)
} # end breakpoints.breakpointfull function
                                    ...
                                    sort = NULL, format.times = NULL, ...)
{
  ...
  return(RVAL)
} # end of summary.breakpointsfull
                                    ...
                                    het.reg = TRUE, het.err = TRUE, vcov. = NULL, sandwich = TRUE, ...)
{
  ...
  return(RVAL)
} # end of confint.breakpointsfull
...
magnitude.breakpointsfull <- function(object, object2,  interval = 0.1, breaks = NULL, component = "trend", ...) # add external object2
{
  ...
  # Assume you have a time series with 100 observations, and maginterval is set to 0.1 (10%). When a breakpoint is detected at the 50th observation,
  # the magnitude will be computed over an interval that spans 10% of the series length, which is 10 observations.
  # This means the magnitude will be calculated using data from the 45th to the 55th observation.
