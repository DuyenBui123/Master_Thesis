...
UseBFASTLite <-  function(InputTS, scrange=NULL, scsig=0.05, breaks="LWZ",
                          ...
                          coefthresholds=c(0, 0), plot=TRUE, quiet=FALSE, order=3,
                          ...
                          seasonfreq=0.5, breaknumthreshold=Inf, altformula=NULL, ...)
{
  ...
  if (Observations > h*2) {
    ...
    if (length(bpOptim$breakpoints) > 0 && !all(is.na(bpOptim$breakpoints))) {
      ...
      bpMag <- magnitude(bp, bpp, breaks=breaks, interval=maginterval, component=magcomponent)$Mag ## add external bpp
    ...
    if (length(bp_rmse) >2) {
      ...
      for (break_nr in 1:(length(bp_rmse)-2)) {
        ...
        #cold_rmse <- c(final_rmse, cold_rmse)
        vec_change <- c(vec_change, abs(bpMag[[break_nr, "diff"]])/bpMag[[break_nr, "RMSD"]])
        bp_datetime <- c(bp_datetime, bpp$datetime[bpOptim$breakpoints[break_nr]])
      }
    ...
    if (length(vec_change) > 0) {
      ...
      Result <- data.frame("Breakpoint" = bpp[bpOptim$breakpoints, "time"], 
                           ...
                           "Coefficients" = bpCoef, "Rsq" = rsquared, "vecmag" = vec_change, "datetime" = bp_datetime) #, "rmse" = cold_rmse) 
    } else {
      # Convert the output into dates of break
      Result <- data.frame("Breakpoint" = bpp[bpOptim$breakpoints, "time"], 
                           "Magnitude" = bpMag, # Now all output of bpMag will be included in a new column
                           ...
                           # in case you want to output only 
                           # selected magnitude results, use column indexing, like: bpMag[,3]
                           "Coefficients" = bpCoef, "Rsq" = rsquared,  "vecmag" = NA, "datetime" =  NA) #, "rmse" = NA) 
    }
    ...
    if (length(Result) < 1){
      return(Result <- data.frame("Breakpoint" = -9999,  "Magnitude" = 0, "Coefficients" = 0))} # If we filtered out all results, return FALSE again
    ...
    if (!quiet){
      print("too cloudy")}
    return(Result <- data.frame("Breakpoint" = -9999,  "Magnitude" = 0, "Coefficients" = 0))
  }
}
...
# aggregate_all_bands <- function(SR_breakpoints,SR_cal_datetime, prob, win ) {
#   threshold <- qnorm(prob, mean=0, sd=6)
...
#         end_date <- SR_cal_datetime1[i+win]
#       }
#       }
#       
...
#         RMSD <- c(RMSD, bp_sp1_bands[[i]]$vecmag[1])
#       }
#       
#       vec_mag_matrix <- matrix(RMSD,nrow = 1,ncol = length(RMSD))
#       sum_vecmag <- sum((vec_mag_matrix))
...
#         } else {final_bp[bp_sp1_bands[[max_key]]$sample_id[1]] <- bp_sp1_bands[[max_key]]$Breakpoint[1] # if new id, save a new key otherwise save in the existed key
#         }
#         
...
#       } # end while loop
#     } else {if (record == 0) { 
#       final_bp[sample_id_nr] <- "not included" # sample id that got filtered out due to -9999 or NA. ONly sample id that contain breaks are remained
#       next } else {
#         next
#       }}
#   } # end for loop
